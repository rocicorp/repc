...
extern "C" {
    ...
    #[wasm_bindgen(method, catch)]
    async fn rollback(this: &JsWrite) -> std::result::Result<(), JsValue>;
}
...
impl Store for JsStore {
    ...
    async fn write<'a>(&'a self, _lc: LogContext) -> Result<Box<dyn Write + 'a>> {
        let x = self.write_impl().await?;
        let x2 = x.unchecked_into::<JsWrite>();
        Ok(Box::new(JsWriteProxy::new(x2)))
    }
